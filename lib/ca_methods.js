...
var lambdaUpdate = function(changes, allFiles, doNotDelete) {
    ...
    Object.keys(allFiles).forEach(function(x) {
        if (allFiles[x] === null) {
            if (!doNotDelete) {
                toDelete.push(FROM_CLOUD_PREFIX + x);
                delete allFiles[x];
            }
        } else {
...
var patchFiles = function(self, commits, changes) {
    commits.forEach(function(x) {
        ...
        p.forEach(function(y) {
            ...
            if (y.newFileName === TOMBSTONE) {
                var fileName = y.oldFileName.split('/').slice(1).join('/');
                self.state.allFiles[fileName] = null;
                self.$.log.debug('>>Deleting: ' + fileName);
            } else {
                fileName = y.newFileName.split('/').slice(1).join('/');
...
var patchGPIO = function(self) {
    ...
    allGPIO.forEach(function(x) {
        ...
        if (x.indexOf(GPIO_PREFIX) === 0) {
            ...
            if (self.state.allFiles[x] === null) {
                pinChanges.push({pin: pin, delete: true});
            } else {
                ...
                if (newMode || (self.state.pinOutputsValue[pin] !== value)) {
                    pinChanges.push ({
                        pin: pin, value: value, newMode: newMode
                    });
                }
            }
        }
        ...
        if ((commits.length > 0) &&
            ...
            async.eachSeries(pinChanges, function(pc, cb0) {
                ...
                if (err) {
                    ...
                    notifyWebApp(self, 'Lambda update');
