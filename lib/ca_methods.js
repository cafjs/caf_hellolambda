    ...
    'handleLambda' : function(changes,  cb) {
        ...
        commits.forEach(function(x) {
            ...
            p.forEach(function(y) {
                var key = y.newFileName.split('/').slice(1).join('/');
                var target = self.state.allFiles[key] || '';
                self.state.allFiles[key] = diff.applyPatch(target, y);
            });
        ...
        async.eachSeries(pinChanges, function(pc, cb0) {
            ...
            if (err) {
                ...
                cb(null, lambdaUpdate(changes, self.allFiles)); 
            }
        });
    },
